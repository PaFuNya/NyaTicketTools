...
def generate_btb_config(accounts, tickets, dry_run=False):
    ...
    global_cookie = accounts[0].get("cookie", "") if accounts else ""
    if not global_cookie:
        err("No cookie available for biliTickerBuy config generation.")
        return
    ...
    # Find the matching account for each ticket
    account_map = {a["name"]: a for a in accounts}

    for ticket in btb_tickets:
        acc_name = ticket.get("account", "main")
        acc = account_map.get(acc_name, accounts[0])
        cookie_str = acc.get("cookie", global_cookie)
    ...
    if btb_tickets:
        ...
        acc_name = first_ticket.get("account", "main")
        acc = account_map.get(acc_name, accounts[0])
        cookie_str = acc.get("cookie", global_cookie)
